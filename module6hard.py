...
class Figure:
    ...
    def set_sides(self, *new_sides):
        if self.__is_valid_sides(*new_sides):
            ...
            self._update_related_attributes()  # Обновляем зависимые атрибуты
    ...
    # Метод-заглушка для обновления зависимых атрибутов
    def _update_related_attributes(self):
        pass
...
class Circle(Figure):
    ...
    # Перерасчитываем радиус при изменении стороны
    def _update_related_attributes(self):
        self.__radius = self.get_sides()[0] / (2 * math.pi)
...
class Cube(Figure):
    ...
    def get_volume(self):
        if len(set(self.get_sides())) > 1:
            raise ValueError("Все стороны куба должны быть равны")
